...
| v0.1 | Milos Jovanovik, Carl Blakeley, Adrian Gschwend, Reto Gmür, Rupert Westenthaler, Johannes Hercher | 26.06.2015 | Pre-review version |
| v0.2 | Milos Jovanovik, Carl Blakeley, Adrian Gschwend, Reto Gmür, Rupert Westenthaler, Johannes Hercher | 08.07.2015 | Reviewed version |
...
Beyond providing a basis for large scale testing of the FP3 platform, the main goal from the user's perspective of this application is to make GNDs identifiers more usable, and to enhance retrieval of publications in libraries. In detail, this leads to the following sub-goals:
...
In order to achieve the goal, the following technical steps are necessary:
...
Additionally, it is counted how often a concept co-occurs with the given GND-ID to compute the distance measure. The distance `d` of a base concept `Cb` (i.e. Hanns Eisler) to a co-occurrent concept `Cx` is computed as the percentage of the total record count having `Cb`.
...
The success metrics described in the DoW for milestone M6, of which Deliverable D5.4 is a part, stipulate "90% of the data stored by the project are interlinked correctly between each other and with public datasets from the Linked Open Data Cloud". To this end, the LKC use case will measure the degree of interlinking achieved between the GND, B3Kat and DBpedia datasets. Please note that we may only evaluate entities that pass a certain degree of threshold to be co-occurrent (interlinked), as stated in [step 4](#UseCaseTaskDetails4), and may limit our evaluation to GND-IDs, since they're thoroughly used in the B3Kat dataset (bibliographic records).

In order to measure the success rate of the newly created interlinkings, we will use a random subset of GND entities for which we will (a) count the number of newly created interlinkings to other entities from the same and other datasets, (b) count the number of successful, i.e. correct interlinkings. From (a) and (b) we can get the percentage of correct interlinkings, i.e. the success rate of the interlinking, for the selected subset.

In order to calculate (a), we will use SPARQL queries. Depending on the RDF properties which will be used for interlinking, we may need to count and identify the pre-existing links first, and then discover the newly created ones, after the datasets are processed by the FP3 Platform. If the RDF properties for new interlinkings are chosen to be unique, the count can be calculated at the end of the process.

On the other hand, calculating (b) will require manual assessment from the use case partners. They will personally assess if each of the newly created interlinkings for a given entity is *useful* or *contextual*, which will be treated as a *successful interlinking*. Because of the manual approach in this part of the validation, we will have to keep the random entity subset rather small. We are currently proposing a subset of 30-50 random GND entities.

From the random nature of the entity subset we can then conclude that the same success rate of interlinking is achieved for all entities from the original dataset. 

As stated in the DoW, we will strive for a success rate of at least 90%.

###Dictionary Matching

In the selected use case for large scale technical validation, an important part played by the FP3 Platform is the dictionary matching. Even though it is not required and specified in the DoW, another metric we can use for validating the platform against the LKC use case would be to evaluate (a) the success rate of the dictionary matching and (b) the enrichment rate of the dictionary matching.

####Success Rate

For (a), we will measure the overlap of GND entities found by the platform in titles, texts, plain text keywords and DBpedia abstracts, with the actual subject heading assigned by librarians. A high percentage of overlap would mean that the FP3 Platform is good enough to be used by librarians as a helping tool - it will be able to find most of the GND entities which a librarian would otherwise add manually. Please note that the stakeholder does not expect to find all entities assigned to a record, because subject librarians often rephrase the publications' contents to intentionally add aspects that are explicitly not mentioned in the title, table of content or abstract. This is why we will use the overlap percentage as a measure for success rate. It allows to draw conclusions about how many entities could be assigned by a 'click-and-go' approach in the future, rather than lookup of concepts by their name or synonyms.  

We expect to have many false positives, as well, due to the lack of disambiguation capabilities in the platform. But, they are not to be seen as errors - they can be provided to the librarians as suggestions which she can choose from or join them to a single entity, such as multiple instances of "Peter Schmidt" (a common name in Germany). Again, this is in line with the use case partners' intent to use the FP3 Platform for support, not as a completely automated replacement.

For this reason, only the true positive matches will be counted.

We expect a success rate of 60-70%.

####Enrichment Rate

For (b), we can count how many titles are enriched with GND-IDs after the dictionary matching. This is useful to the use case partners due to the fact that many of their older records - published and catalogued before the 1980s - do not have GND-IDs assigned, but only plain text keywords (`dcterms:subject`). Since around 30-40% of the records in B3Kat are estimated to be in this state, having explicit links to GND-IDs would be of great use for them.

In order to assess the enrichment rate, we will count how many titles are correctly linked with GND-IDs after the datasets have been processed with the FP3 Platform. This will also be done manually, having the use case partners assess the correctness of the GND-IDs for each selected title. The incorrect links to GND-IDs will be not taken into account, as they are expected (as in (a)) and can be manually filtered out. Because the `dcterms:subject` property contains keywords in various languages, we will only take german words into account, as GND is a german language authority file. The percentage of correct GND-IDs found is measured manually by the stakeholders, who expect to find around 70-80% of the links created by the GND - dictionary annotator to be correct. In this step the dictionary annotator runs only on the datatype property `dcterms:subject` of the records that have no GND-IDs linked before processing. False positives will not be taken into account due to missing disambiguation capabilities. The count of enriched records with at least one *new* interlinking to GND is assesed via SPARQL, after the enrichment process. Depending on the RDF properties which will be used for interlinking by dictionary matching, we may need to count and identify the pre-existing links from titles to GND-IDs first, before processing.

Because of the manual evaluation of the correctness of linked GND-IDs, we will select a random subset of 30-50 titles, for which the assessment will be done. Based on the randomness of the title subset, we can conclude that the same success rate for enrichment is valid for all titles in the dataset.
