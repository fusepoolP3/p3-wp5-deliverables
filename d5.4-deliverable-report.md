...
For (a), we will measure the overlap of GND entities found by the platform in titles, texts, plain text keywords and DBpedia abstracts, with the actual subject heading assigned by librarians. A high percentage of overlap would mean that the FP3 Platform is good enough to be used by librarians as a helping tool - it will be able to find most of the GND entities which a librarian would otherwise add manually. Please note that the stakeholder does not expect to find all entities assigned to a record, because subject librarians often rephrase the publications' contents to intentionally add aspects that are explicitly not mentioned in the title, table of contents or abstract. This is why we will use the overlap percentage as a measure for success rate. It allows one to draw conclusions about how many entities could be assigned by a 'click-and-go' approach in the future, rather than lookup of concepts by their name or synonyms.  
