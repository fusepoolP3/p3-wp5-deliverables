...
- [Validation Metrics](#ValidationMetrics)
  - [Interlinking Quality](#InterlinkingQuality)
    - [Interlinking: GND - B3Kat](InterlinkingGNDB3Kat)
    - [Interlinking: GND - DBpedia](InterlinkingGNDDBpedia)
  - [Feasibility for Contextual Links](FeasibilityForContextLinks)
...
This use case focuses on exercising the bulk import and bulk transformation (annotation) features of the platform, along with the ability to handle large datasets. The components of the FP3 platform which will be tested include:
...
LKC supports libraries in data analysis and interlinking to (a) collect complementary information, (b) compute keyword co-occurrences, and (c) to build word contexts of library keywords. The proposed clusters can be harnessed to suggest additional search terms or related documents to the end users, but also to gain complementary terms during subject indexing. LKC provides an aggregated dataset that is enriched with co-occurrence links, and which can hardly be computed without semantic technologies running on high-end hardware. Future plans could focus on workflow maintenance, e.g. including more resources and triggered runs on updates within the connected datasets.
...
No special component where used for the large scale validation

<a name="FP3PlatformConfiguration"></a> 
#### FP3 Platform Configuration

The configuration for the Large Scale Validation consists of several transforming LDPC
configured to use [Literal Extraction Transformer](https://github.com/fusepoolP3/p3-literal-extraction-transformer)
...
The enriched GND instances (result of the pre-processing steps 1-5) are ingested to a transforming LDPC
responsible for enriching those data with extraced named entities as well as links to IPTC, TheSoc and STW.
...
configured to read literal values from `nif:isString`, `dbo:abstract`, `gnd:publication` and `biographicalOrHistoricalInformation`. Those literal values are collected per language, concatenated and sent to a Apache Stanbol Transformer configured with to extract Named Entities and find Mentions of Entities of IPTC, TheSoc and STW. The text analysis results encoded using the [Fuespool Annotation Model](https://github.com/fusepoolP3/overall-architecture/blob/master/wp3/fp-anno-model/fp-anno-model.md) (FAM) are used by the Literal Extraction Transformer to enrich the processed GND entities.

Extracted Named Entities are stored using `fam:preson-ne-reference` for persons, `fam:organization-ne-reference` for Organizations, `fam:location-ne-reference` for Locations and `fam:named-entity-reference` for other detected named entity types. Entity mentions detected for IPTC, TheSoc or STW are linked by using `fam:entity-reference`. This means that GND instances will have values for those properties representing the information extraction results for the literal values of the input properties.


__B3Kat Configuration:__

For the processing of B3Kat instances a similar processing workflow of a Literal Extraction Transformer coupled with a Stanbol Transformer is used. For validation purpose the use case partner requested to process B3Kat resource with two different configurations.

1. Enriching B3Kat data with Named Entity Extraction, IPTC, TheSoc and STW: This configuration uses the Title, Sub-Title, subjects as input and sends it to the Apache Stanbol Transformer configuration configured for the mentioned features. This is the same Stanbol Transformer also used to process text extracted from the enriched GND entities.

2. Linking B3Kat subject annotation with GND: This configuration aims to link those ~40% of B3Kat instances that where not manually taged with GND instances but only with String keywords. For that a Stanbol Transformer was configured that links against the current GND version. As input only `dc:subject` values of B3Kat instances are used. (NOTE: GND subjects for B3Kat are stored in `dct:subject`; `dc:subject` is used for String subjects).
...
This describes the configuration used for extracting information of concatenated literal values sent by the Literal Extraction Transformer. As those content is mainly constituted by single labels and only partially written text (e.g. DBPedia abstract of GND instances with a link ot DBPedia) the information extraction task is especially hard.

The lange scale validation includes Named Entity Recognition (NER). However as the user NER models where trained mainly on news texts the performance on concatenated labels was expected to be poor. For the test the German NER model provided by the [IXA Pipe NERC](https://github.com/ixa-ehu/ixa-pipe-nerc/) where used. All IXA Pipe NERC models are available in the [Fusepool Stanbol Launcher](https://github.com/fusepoolP3/p3-stanbol-launcher).

The large scale validation uses four datasets for Entity Linking. This means that mentions of Entities are searched in processed text. If such mentions are found the document is linked with that entity. Enriched GND resources are linked with the following three vocabularies. 

1. The [Thesaurus for the Social Sciences](http://www.gesis.org/en/services/research/thesauri-und-klassifikationen/social-science-thesaurus/) ([The Soz](http://www.semantic-web-journal.net/sites/default/files/swj279_2.pdf)).
...
For processing the B3Kat `dc:subject` values lining against GND was used in addition to those three vocabularies.

As part of the large scale validation pre-indexed versions of all those vocabularies where created by Fuespool and are included in the default [Fusepool Stanbol Launcher](https://github.com/fusepoolP3/p3-stanbol-launcher). Because of the size of the dataset the index data of GND are not included but need to be downloaded and installed manually.

Predefined configuration include three linking modes: 
...
Some of the use case tasks, primarily preparation of the input data in tasks 1 - 5, are to be performed by the application stakeholders (primarily Johannes Hercher, Free University Berlin). Tasks 6 - 8 will be performed by the FP3 platform itself and constitute the core of the validation testing. These latter tasks comprise NER on selected document texts, matching thesauri concepts to each GND and matching against DBpedia using a combination of the following FP3 transformers: the Stanbol Enhancer Transformer, the Dictionary Matching Transformer, the dataTXT Transformer and the Pipeline Transformer. These transformers are detailed in the D3.1 Deliverable Report. Appendix B below contains introductory notes on Apache Stanbol.
...
##Validation Metrics
The success metrics described in the DoW for milestone M6, of which Deliverable D5.4 is a part, stipulate "90% of the data stored by the project are interlinked correctly between each other and with public datasets from the Linked Open Data Cloud". To this end, the LKC use case will measure the degree of interlinking achieved between the GND, B3Kat and DBpedia datasets. The large scale validation will be evaluated according to **quality** (by means of recall and precision) and **feasibility** (by means of supporting future exploration and recommendation features in libraries). Metrics about the platforms performance (i.e. how long do the chains take for processing, etc.) are not taken into account. The remainder of this section is split into the validation of interlinking (creating additional links prior to already existing relations), and dictionary matching which aims to create new co-occurence relations.

<a name="InterlinkingQuality"></a>
###Interlinking Quality
The datasets used in the LKC (DBpedia, GND and B3Kat) are already linked to a certain degree. The aim of this evaluation is to check whether it can be improved using the platform.

<a name="InterlinkingGNDB3Kat"></a>
#### Interlinking: GND - B3Kat
Within the LKC the dictionary matching is used to find appropriate GND-IDs for texts in B3Kat's bibliographic data (records). The stakeholder wants to know if this processing is feasible to provide a more coherent dataset. Many of their older records - published and catalogued before the 1980s - do not have GND-IDs assigned, but only plain text keywords (`dcterms:subject`). Around 30-40% of the records in B3Kat are estimated to be in this state, having explicit links to GND-IDs would be of great use to them. We will count how many keywords within the bibliographic records are correctly linked with GND-IDs, after the datasets have been processed with the FP3 Platform. This will be done manually, by having the use case partners assess the correctness of the GND-IDs for each selected record. To calculate the overlap rate, only records are assessed that had no pre-existing GND Links: 

    Overlap (in %) = ( A / B ) * 100
    where:
    A: Count of strings in `dcterms:subject` for one record in German language.
    B: Count of correct matchings to GND-IDs for one record.


Incorrect links to GND-IDs will be not be taken into account, because the GND Authority File does not contain unique names. Peoples, locations and corporations often share the same names/labels. It is the intention of the stakeholder to display them in future interfaces to support manual indexing. False entities can be easily filtered out by a human using the GND's inherent type (`rdf:type`) and [subject classification]( http://d-nb.info/standards/vocab/gnd/gnd-sc.html). Because the `dcterms:subject` property of B3Kat records contains keywords in various languages, we will only take German words into account. The percentage of correct GND-IDs found is measured manually by the stakeholders, who expect to find around 70-80% of the links created by the GND - dictionary annotator to be correct. We will select a random subset of 30-50 bibliographic records from B3Kat, for which the assessment will be done. Based on the randomness of the record subset, we can conclude that the same success rate for enrichment is valid for all titles in the dataset.

<a name="InterlinkingGNDDBpedia"></a>
####Interlinking: GND - DBpedia 

Wikipedia and DBpedia include links to GND-IDs. This was manually done by humans in Wikipedia. In this part of the  evaluation we will test if additional links can be created by the platform. Precision and recall metrics are applied to a set of 50-150 random GNDs that have matched to the German DBpedia (because of the language of the GND authority file). The platform will create custom properties for linking to allow a comparison between pre-existing links and newly created ones. Since the interlinking may be more complete but more error-prone if we take wikipage-redirects into account, two separate runs are created. 
A) Matchings for GND -> DBpedia based on the page title 
B) Matchings for GND -> DBpedia based on page title and wikipage-redirects. 

We will create a gold standard examining 50 GND-IDs for a possible linkage to DBpedia. Afterwards recall and precision is used:

Recall: *Percentage of correct links (c) of the Dictionary Annotator to all possible links manually set in the Gold Standard (g).*

    `recall =  g / c * 100` 
    
Precision: *Percentage of correct links (c) to the count of all links created by the platform (l).* 
    
    `precision =  l / c * 100`
    
<a name="FeasibilityForContextLinks"></a>
### Feasibility for Contextual Links
In the selected use case for large scale technical validation, an important part played by the FP3 Platform is the dictionary matching to create co-occurrence relations. The creation of co-occurrence relations as described in [Section 4](#UseCaseTaskDetails4) is dedicated to support librarians and patrons with novel recommendation features. Due to the nature of serendipity and exploratory search it is not reasonable to determine a fixed area of legitimate interest (Gold Standard). Therefore, we calculate the success rate as follows:

`success rate (in %) = ( A / B ) * 100`
...
This manual evaluation will be done for 30-50 randomly selected GND-IDs.
...
<a name="PlatformPerformance"></a> 
## Platform Performance

We propose assessing the platform's operation in terms of disk, memory and network performance.

### Disk IO / Memory Usage

We will monitor disk IO and memory usage during processing runs in order to see what resources are used. The Linux tools for performing this monitoring still remain to be assessed and selected.
...
In the semantic lifting step, entity linking against the four controlled vocabularies and the dataTXT linking to DBPedia will take place.
