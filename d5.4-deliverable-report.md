...
| v0.2 | Milos Jovanovik, Carl Blakeley, Adrian Gschwend, Reto Gmür, Rupert Westenthaler | 03.07.2015 | Reviewed version |
...
For (a), we will measure the overlap of GND entities found by the platform in titles, texts, plain text keywords and DBpedia abstracts, with the actual assigned subject heading by librarians. A high percentage of overlap would mean that the FP3 Platform is good enough to be used by librarians as a helping tool - it will be able to find most of the GND entities which a librarian would otherwise add manually. Please note that the stakeholder don't expects to find all entities assigned to a record, because subject librarians often rephrase the publications' contents to intentionally add aspects that are explicitly not mentioned in the title,TOC, or abstract. This is why we will use the overlap percentage as a measure for success rate. It allows to draw conclusions about how many entities could be assigned by click-and-go in the future, rather than lookup of concepts by their name or synonyms.  
...
For (b), we can count how many titles are enriched with GND-IDs after the dictionary matching. This is useful to the use case partners due to the fact that many of their older records - published and catalogued before the 1980s - do not have GND-IDs assigned, but only plain text keywords (`dcterms:subject`). Since around 30-40% of records in B3Kat are estimated to be in this state. Having explicit links to GND-IDs would be of great use for them.

In order to assess the success enrichment rate, we will count how many titles are correctly linked with GND-IDs after the datasets have been processed with the FP3 Platform. This will also be done manually, having the use case partners assess the correctness of the GND-IDs for each selected title. The incorrect links to GND-IDs will be not taken into account, as they are expected (as in (a)) and can be manually filtered out. Because the `dcterms:subject` property contains keywords in various languages, we will only take german words into account (GND is a german language Authority file). The percentage of correct found GND-IDs is measured manually by the stakeholder, which expects to find 70-80% links created by the GND-Dictionary Annotator to be correct.  In this step the dictionary annotator runs only on the datatype property `dcterms:subject` of the records that have no GND-URIs linked before processing. False positives wont be taken into account due to missing disambiguation capabilities. The count of enriched records with at least one _new_ interlinking to GND is assesed via SPARQL after the enrichment process.
