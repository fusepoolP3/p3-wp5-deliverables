...
In order to measure the success rate of the newly created interlinkings, we will use a random subset of GND entities for which we will (a) count the number of newly created interlinkings to other entities from the same and other datasets, (b) count the number of successful, i.e. correct such interlinkings. From (a) and (b) we can get the percentage of correct interlinkings, i.e. the success rate of the interlinking, for the selected subset.
...
For (a), we will measure the overlap of GND entities found by the platform in titles, texts, plain text keywords and DBpedia abstracts, with the actual assigned subject heading by librarians. A high percentage of overlap would mean that the FP3 Platform is good enough to be used by librarians as a helping tool - it will be able to find all GND entities which a librarian would otherwise add manually. This is why we will use the overlap percentage as a measure for success rate. 

We expect to have many false positives, as well, due to the lack of disambiguation capabilities in the platform. But, they are not to be seen as errors - they can be provided to the librarians as suggestions which can then be manually filtered out. This is in line with the use case partners' intent to use the FP3 Platform for support.

For this reason, only the true positive matches will be counted.

We expect a success rate of ~X%.

####Enrichment Rate

For (b), we can count how many titles are enriched with GND-IDs after the dictionary matching. This is useful to the use case partners due to the fact that many of their older records - published and catalogued before the 1980s - do not have GND-IDs assigned, but only plan text keywords. Having explicit links to GND-IDs would be of great use for them.

In order to assess the success enrichment rate, we would count how many titles are correctly linked with GND-IDs after the datasets have been processed with the FP3 Platform. This will also be done manually, having the use case partners assess the correctness of the GND-IDs for each selected title. The incorrect links to GND-IDs will be not taken into account, as they are expected (as in (a)) and can be manually filtered out. Having at least one correct GND-ID associated with a given title will count as a successfully enriched title.
...
The expected success rate of enrichment is 30 - 40%, i.e. we expect that 30 - 40% of the titles without GND-IDs will have at least one GND-ID associated with them.
